...
For some reason, the state thinks, that they feel safe there and are open to discussions. 
...
all_tokens = [word.lower() for review in df['review'] for word in review.split()]
...
df['sequence'] = df['review'].apply(lambda x: text_to_sequence(x, vocab, SEQUENCE_LENGTH))
...
all_tokens = [token for review in df['review'] for token in clean_and_tokenize(review)]
token_counts = Counter(all_tokens)

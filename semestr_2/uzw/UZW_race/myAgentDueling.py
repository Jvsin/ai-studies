...
class MyAgent:
    def __init__(self, input_dims=17, n_actions=5, lr=0.0001, gamma=0.99, epsilon=1.0, epsilon_dec=0.995, epsilon_min=0.05):
        ...
        # Use records directory next to this file
        RECORDS_DIR = os.path.join(os.path.dirname(__file__), "records")
        if not os.path.exists(RECORDS_DIR):
            os.makedirs(RECORDS_DIR, exist_ok=True)
        self.path = os.path.join(RECORDS_DIR, "race_model_dhq.pth")
    ...
    def save(self, reward):
        if reward > self.best_reward:
            ...
            records_dir = os.path.dirname(self.path)
            if not os.path.exists(records_dir):
                os.makedirs(records_dir, exist_ok=True)
    ...
    def load(self):
        # Diagnostic: show which path we're checking
        # print(f"Sprawdzam ścieżkę modelu: {self.path}")
        if os.path.exists(self.path):
            try:
                self.policy_net.load_state_dict(torch.load(self.path, map_location=self.device))
                self.target_net.load_state_dict(self.policy_net.state_dict())
                print(f"Model loaded from: {self.path}")
                self.epsilon = 0.7
            except Exception as e:
                print(f"Błąd ładowania modelu z {self.path}: {e}")
                print("Startuję z losową inicjalizacją.")
        else:
            print(f"Brak pliku z wagami pod: {self.path} — startuję od zera.")
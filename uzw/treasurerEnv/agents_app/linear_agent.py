...
class LinearApproxAgent:
    ...
    def _softmax(self, q_values, temperature=1.0):
        exp_q = np.exp((np.array(q_values) - np.max(q_values)) / temperature)
        probs = exp_q / np.sum(exp_q)
        return probs

    def approximation_function(self, state, action):
        ...
        #     target_pos = base_pos
        if treasures:
            distances = [abs(next_x - t[0]) + abs(next_y - t[1]) for t in treasures]
            closest_treasure = min(distances)
    ...
    def _features_approx(self, state, action):
        pos1, pos2, treasures, hold1, hold2 = state
    ...
    def get_q_value(self, state, action):
        features = self.approximation_function(state, action)
